package com.xy2.entity;


public class RoleSummoning {

  private String summoningid;
  private String summoningskin;
  private String stye;
  private String hp;
  private String mp;
  private String ap;
  private String sp;
  private String growlevel;
  private String resistance;
  private String skill;
  private String gold;
  private String wood;
  private String soil;
  private String water;
  private String fire;
  private String summoningname;
  private String sid;
  private String ssn;
  private String roleid;
  private String bone;
  private String spir;
  private String power;
  private String speed;
  private String canpoint;
  private String grade;
  private String exp;
  private String faithful;
  private String friendliness;
  private String price;
  private String basishp;
  private String basismp;
  private String basisap;
  private String basissp;
  private String openseal;
  private String innergoods;
  private String dragon;
  private String extrahp;
  private String extramp;
  private String extraap;
  private String extrasp;
  private String petskills;
  private String turnrount;
  private String gradehp;
  private String grademp;
  private String nedanresistance;
  private String revealnum;
  private String flyupnum;
  private String beastskills;
  private String fourattributes;
  private String skilldata;
  private String lyk;
  private String zqk;
  private String alchemynum;
  private String growupdannum;
  private String colorscheme;
  private String drac;
  private String trainnum;
  private String petlock;
  private String calm;
  private String czjjd;


  public String getSummoningid() {
    return summoningid;
  }

  public void setSummoningid(String summoningid) {
    this.summoningid = summoningid;
  }


  public String getSummoningskin() {
    return summoningskin;
  }

  public void setSummoningskin(String summoningskin) {
    this.summoningskin = summoningskin;
  }


  public String getStye() {
    return stye;
  }

  public void setStye(String stye) {
    this.stye = stye;
  }


  public String getHp() {
    return hp;
  }

  public void setHp(String hp) {
    this.hp = hp;
  }


  public String getMp() {
    return mp;
  }

  public void setMp(String mp) {
    this.mp = mp;
  }


  public String getAp() {
    return ap;
  }

  public void setAp(String ap) {
    this.ap = ap;
  }


  public String getSp() {
    return sp;
  }

  public void setSp(String sp) {
    this.sp = sp;
  }


  public String getGrowlevel() {
    return growlevel;
  }

  public void setGrowlevel(String growlevel) {
    this.growlevel = growlevel;
  }


  public String getResistance() {
    return resistance;
  }

  public void setResistance(String resistance) {
    this.resistance = resistance;
  }


  public String getSkill() {
    return skill;
  }

  public void setSkill(String skill) {
    this.skill = skill;
  }


  public String getGold() {
    return gold;
  }

  public void setGold(String gold) {
    this.gold = gold;
  }


  public String getWood() {
    return wood;
  }

  public void setWood(String wood) {
    this.wood = wood;
  }


  public String getSoil() {
    return soil;
  }

  public void setSoil(String soil) {
    this.soil = soil;
  }


  public String getWater() {
    return water;
  }

  public void setWater(String water) {
    this.water = water;
  }


  public String getFire() {
    return fire;
  }

  public void setFire(String fire) {
    this.fire = fire;
  }


  public String getSummoningname() {
    return summoningname;
  }

  public void setSummoningname(String summoningname) {
    this.summoningname = summoningname;
  }


  public String getSid() {
    return sid;
  }

  public void setSid(String sid) {
    this.sid = sid;
  }


  public String getSsn() {
    return ssn;
  }

  public void setSsn(String ssn) {
    this.ssn = ssn;
  }


  public String getRoleid() {
    return roleid;
  }

  public void setRoleid(String roleid) {
    this.roleid = roleid;
  }


  public String getBone() {
    return bone;
  }

  public void setBone(String bone) {
    this.bone = bone;
  }


  public String getSpir() {
    return spir;
  }

  public void setSpir(String spir) {
    this.spir = spir;
  }


  public String getPower() {
    return power;
  }

  public void setPower(String power) {
    this.power = power;
  }


  public String getSpeed() {
    return speed;
  }

  public void setSpeed(String speed) {
    this.speed = speed;
  }


  public String getCanpoint() {
    return canpoint;
  }

  public void setCanpoint(String canpoint) {
    this.canpoint = canpoint;
  }


  public String getGrade() {
    return grade;
  }

  public void setGrade(String grade) {
    this.grade = grade;
  }


  public String getExp() {
    return exp;
  }

  public void setExp(String exp) {
    this.exp = exp;
  }


  public String getFaithful() {
    return faithful;
  }

  public void setFaithful(String faithful) {
    this.faithful = faithful;
  }


  public String getFriendliness() {
    return friendliness;
  }

  public void setFriendliness(String friendliness) {
    this.friendliness = friendliness;
  }


  public String getPrice() {
    return price;
  }

  public void setPrice(String price) {
    this.price = price;
  }


  public String getBasishp() {
    return basishp;
  }

  public void setBasishp(String basishp) {
    this.basishp = basishp;
  }


  public String getBasismp() {
    return basismp;
  }

  public void setBasismp(String basismp) {
    this.basismp = basismp;
  }


  public String getBasisap() {
    return basisap;
  }

  public void setBasisap(String basisap) {
    this.basisap = basisap;
  }


  public String getBasissp() {
    return basissp;
  }

  public void setBasissp(String basissp) {
    this.basissp = basissp;
  }


  public String getOpenseal() {
    return openseal;
  }

  public void setOpenseal(String openseal) {
    this.openseal = openseal;
  }


  public String getInnergoods() {
    return innergoods;
  }

  public void setInnergoods(String innergoods) {
    this.innergoods = innergoods;
  }


  public String getDragon() {
    return dragon;
  }

  public void setDragon(String dragon) {
    this.dragon = dragon;
  }


  public String getExtrahp() {
    return extrahp;
  }

  public void setExtrahp(String extrahp) {
    this.extrahp = extrahp;
  }


  public String getExtramp() {
    return extramp;
  }

  public void setExtramp(String extramp) {
    this.extramp = extramp;
  }


  public String getExtraap() {
    return extraap;
  }

  public void setExtraap(String extraap) {
    this.extraap = extraap;
  }


  public String getExtrasp() {
    return extrasp;
  }

  public void setExtrasp(String extrasp) {
    this.extrasp = extrasp;
  }


  public String getPetskills() {
    return petskills;
  }

  public void setPetskills(String petskills) {
    this.petskills = petskills;
  }


  public String getTurnrount() {
    return turnrount;
  }

  public void setTurnrount(String turnrount) {
    this.turnrount = turnrount;
  }


  public String getGradehp() {
    return gradehp;
  }

  public void setGradehp(String gradehp) {
    this.gradehp = gradehp;
  }


  public String getGrademp() {
    return grademp;
  }

  public void setGrademp(String grademp) {
    this.grademp = grademp;
  }


  public String getNedanresistance() {
    return nedanresistance;
  }

  public void setNedanresistance(String nedanresistance) {
    this.nedanresistance = nedanresistance;
  }


  public String getRevealnum() {
    return revealnum;
  }

  public void setRevealnum(String revealnum) {
    this.revealnum = revealnum;
  }


  public String getFlyupnum() {
    return flyupnum;
  }

  public void setFlyupnum(String flyupnum) {
    this.flyupnum = flyupnum;
  }


  public String getBeastskills() {
    return beastskills;
  }

  public void setBeastskills(String beastskills) {
    this.beastskills = beastskills;
  }


  public String getFourattributes() {
    return fourattributes;
  }

  public void setFourattributes(String fourattributes) {
    this.fourattributes = fourattributes;
  }


  public String getSkilldata() {
    return skilldata;
  }

  public void setSkilldata(String skilldata) {
    this.skilldata = skilldata;
  }


  public String getLyk() {
    return lyk;
  }

  public void setLyk(String lyk) {
    this.lyk = lyk;
  }


  public String getZqk() {
    return zqk;
  }

  public void setZqk(String zqk) {
    this.zqk = zqk;
  }


  public String getAlchemynum() {
    return alchemynum;
  }

  public void setAlchemynum(String alchemynum) {
    this.alchemynum = alchemynum;
  }


  public String getGrowupdannum() {
    return growupdannum;
  }

  public void setGrowupdannum(String growupdannum) {
    this.growupdannum = growupdannum;
  }


  public String getColorscheme() {
    return colorscheme;
  }

  public void setColorscheme(String colorscheme) {
    this.colorscheme = colorscheme;
  }


  public String getDrac() {
    return drac;
  }

  public void setDrac(String drac) {
    this.drac = drac;
  }


  public String getTrainnum() {
    return trainnum;
  }

  public void setTrainnum(String trainnum) {
    this.trainnum = trainnum;
  }


  public String getPetlock() {
    return petlock;
  }

  public void setPetlock(String petlock) {
    this.petlock = petlock;
  }


  public String getCalm() {
    return calm;
  }

  public void setCalm(String calm) {
    this.calm = calm;
  }


  public String getCzjjd() {
    return czjjd;
  }

  public void setCzjjd(String czjjd) {
    this.czjjd = czjjd;
  }

}
