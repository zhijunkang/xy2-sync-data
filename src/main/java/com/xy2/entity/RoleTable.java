package com.xy2.entity;


public class RoleTable {

  private String roleId;
  private String userId;
  private String speciesId;
  private String summoningId;
  private String gangId;
  private String mountId;
  private String troopId;
  private String raceId;
  private String skillId;
  private String boothId;
  private String taskId;
  private String hp;
  private String mp;
  private String gold;
  private String codecard;
  private String experience;
  private String grade;
  private String prestige;
  private String pkrecord;
  private String rolename;
  private String title;
  private String path;
  private String sex;
  private String localname;
  private String skill;
  private String X;
  private String Y;
  private String mapid;
  private String uptime;
  private String gangpost;
  private String achievement;
  private String contribution;
  private String bone;
  private String spir;
  private String power;
  private String speed;
  private String canpoint;
  private String captain;
  private String savegold;
  private String password;
  private String gangname;
  private String havebaby;
  private String newrole;
  private String racename;
  private String maxexp;
  private String marryobject;
  private String skills;
  private String timinggood;
  private String turnaround;
  private String taskdaily;
  private String born;
  private String resistance;
  private String servermestring;
  private String taskreceive;
  private String taskcomplete;
  private String taskdata;
  private String fighting;
  private String dbexp;
  private String score;
  private String kill;
  private String babyid;
  private java.sql.Date drawing;
  private String calm;
  private String xiuwei;
  private String extrapoint;
  private String paysum;
  private String daypaysum;
  private String dayandpayorno;
  private String dayfirstinorno;
  private String daygetorno;
  private String vipget;
  private String loworhihtpack;
  private String pals;
  private String fmsld;
  private String attachpack;
  private String meridians;
  private String hjmax;
  private String qianDao;
  private String onlineTime;
  private String xingpans;
  private String ttvictory;
  private String ttfail;
  private String ttrecord;
  private String ttjiangli;
  private String gmshoptype;
  private String gradeincrease;
  private String currentattribute;
  private String gametimeremaining;
  private String shigongx;

  public String getShigongx() {
    return shigongx;
  }

  public void setShigongx(String shigongx) {
    this.shigongx = shigongx;
  }




  public String getRoleId() {
    return roleId;
  }

  public void setRoleId(String roleId) {
    this.roleId = roleId;
  }


  public String getUserId() {
    return userId;
  }

  public void setUserId(String userId) {
    this.userId = userId;
  }


  public String getSpeciesId() {
    return speciesId;
  }

  public void setSpeciesId(String speciesId) {
    this.speciesId = speciesId;
  }


  public String getSummoningId() {
    return summoningId;
  }

  public void setSummoningId(String summoningId) {
    this.summoningId = summoningId;
  }


  public String getGangId() {
    return gangId;
  }

  public void setGangId(String gangId) {
    this.gangId = gangId;
  }


  public String getMountId() {
    return mountId;
  }

  public void setMountId(String mountId) {
    this.mountId = mountId;
  }


  public String getTroopId() {
    return troopId;
  }

  public void setTroopId(String troopId) {
    this.troopId = troopId;
  }


  public String getRaceId() {
    return raceId;
  }

  public void setRaceId(String raceId) {
    this.raceId = raceId;
  }


  public String getSkillId() {
    return skillId;
  }

  public void setSkillId(String skillId) {
    this.skillId = skillId;
  }


  public String getBoothId() {
    return boothId;
  }

  public void setBoothId(String boothId) {
    this.boothId = boothId;
  }


  public String getTaskId() {
    return taskId;
  }

  public void setTaskId(String taskId) {
    this.taskId = taskId;
  }


  public String getHp() {
    return hp;
  }

  public void setHp(String hp) {
    this.hp = hp;
  }


  public String getMp() {
    return mp;
  }

  public void setMp(String mp) {
    this.mp = mp;
  }


  public String getGold() {
    return gold;
  }

  public void setGold(String gold) {
    this.gold = gold;
  }


  public String getCodecard() {
    return codecard;
  }

  public void setCodecard(String codecard) {
    this.codecard = codecard;
  }


  public String getExperience() {
    return experience;
  }

  public void setExperience(String experience) {
    this.experience = experience;
  }


  public String getGrade() {
    return grade;
  }

  public void setGrade(String grade) {
    this.grade = grade;
  }


  public String getPrestige() {
    return prestige;
  }

  public void setPrestige(String prestige) {
    this.prestige = prestige;
  }


  public String getPkrecord() {
    return pkrecord;
  }

  public void setPkrecord(String pkrecord) {
    this.pkrecord = pkrecord;
  }


  public String getRolename() {
    return rolename;
  }

  public void setRolename(String rolename) {
    this.rolename = rolename;
  }


  public String getTitle() {
    return title;
  }

  public void setTitle(String title) {
    this.title = title;
  }


  public String getPath() {
    return path;
  }

  public void setPath(String path) {
    this.path = path;
  }


  public String getSex() {
    return sex;
  }

  public void setSex(String sex) {
    this.sex = sex;
  }


  public String getLocalname() {
    return localname;
  }

  public void setLocalname(String localname) {
    this.localname = localname;
  }


  public String getSkill() {
    return skill;
  }

  public void setSkill(String skill) {
    this.skill = skill;
  }


  public String getX() {
    return X;
  }

  public void setX(String X) {
    this.X = X;
  }


  public String getY() {
    return Y;
  }

  public void setY(String Y) {
    this.Y = Y;
  }


  public String getMapid() {
    return mapid;
  }

  public void setMapid(String mapid) {
    this.mapid = mapid;
  }


  public String getUptime() {
    return uptime;
  }

  public void setUptime(String uptime) {
    this.uptime = uptime;
  }


  public String getGangpost() {
    return gangpost;
  }

  public void setGangpost(String gangpost) {
    this.gangpost = gangpost;
  }


  public String getAchievement() {
    return achievement;
  }

  public void setAchievement(String achievement) {
    this.achievement = achievement;
  }


  public String getContribution() {
    return contribution;
  }

  public void setContribution(String contribution) {
    this.contribution = contribution;
  }


  public String getBone() {
    return bone;
  }

  public void setBone(String bone) {
    this.bone = bone;
  }


  public String getSpir() {
    return spir;
  }

  public void setSpir(String spir) {
    this.spir = spir;
  }


  public String getPower() {
    return power;
  }

  public void setPower(String power) {
    this.power = power;
  }


  public String getSpeed() {
    return speed;
  }

  public void setSpeed(String speed) {
    this.speed = speed;
  }


  public String getCanpoint() {
    return canpoint;
  }

  public void setCanpoint(String canpoint) {
    this.canpoint = canpoint;
  }


  public String getCaptain() {
    return captain;
  }

  public void setCaptain(String captain) {
    this.captain = captain;
  }


  public String getSavegold() {
    return savegold;
  }

  public void setSavegold(String savegold) {
    this.savegold = savegold;
  }


  public String getPassword() {
    return password;
  }

  public void setPassword(String password) {
    this.password = password;
  }


  public String getGangname() {
    return gangname;
  }

  public void setGangname(String gangname) {
    this.gangname = gangname;
  }


  public String getHavebaby() {
    return havebaby;
  }

  public void setHavebaby(String havebaby) {
    this.havebaby = havebaby;
  }


  public String getNewrole() {
    return newrole;
  }

  public void setNewrole(String newrole) {
    this.newrole = newrole;
  }


  public String getRacename() {
    return racename;
  }

  public void setRacename(String racename) {
    this.racename = racename;
  }


  public String getMaxexp() {
    return maxexp;
  }

  public void setMaxexp(String maxexp) {
    this.maxexp = maxexp;
  }


  public String getMarryobject() {
    return marryobject;
  }

  public void setMarryobject(String marryobject) {
    this.marryobject = marryobject;
  }


  public String getSkills() {
    return skills;
  }

  public void setSkills(String skills) {
    this.skills = skills;
  }


  public String getTiminggood() {
    return timinggood;
  }

  public void setTiminggood(String timinggood) {
    this.timinggood = timinggood;
  }


  public String getTurnaround() {
    return turnaround;
  }

  public void setTurnaround(String turnaround) {
    this.turnaround = turnaround;
  }


  public String getTaskdaily() {
    return taskdaily;
  }

  public void setTaskdaily(String taskdaily) {
    this.taskdaily = taskdaily;
  }


  public String getBorn() {
    return born;
  }

  public void setBorn(String born) {
    this.born = born;
  }


  public String getResistance() {
    return resistance;
  }

  public void setResistance(String resistance) {
    this.resistance = resistance;
  }


  public String getServermestring() {
    return servermestring;
  }

  public void setServermestring(String servermestring) {
    this.servermestring = servermestring;
  }


  public String getTaskreceive() {
    return taskreceive;
  }

  public void setTaskreceive(String taskreceive) {
    this.taskreceive = taskreceive;
  }


  public String getTaskcomplete() {
    return taskcomplete;
  }

  public void setTaskcomplete(String taskcomplete) {
    this.taskcomplete = taskcomplete;
  }


  public String getTaskdata() {
    return taskdata;
  }

  public void setTaskdata(String taskdata) {
    this.taskdata = taskdata;
  }


  public String getFighting() {
    return fighting;
  }

  public void setFighting(String fighting) {
    this.fighting = fighting;
  }


  public String getDbexp() {
    return dbexp;
  }

  public void setDbexp(String dbexp) {
    this.dbexp = dbexp;
  }


  public String getScore() {
    return score;
  }

  public void setScore(String score) {
    this.score = score;
  }


  public String getKill() {
    return kill;
  }

  public void setKill(String kill) {
    this.kill = kill;
  }


  public String getBabyid() {
    return babyid;
  }

  public void setBabyid(String babyid) {
    this.babyid = babyid;
  }


  public java.sql.Date getDrawing() {
    return drawing;
  }

  public void setDrawing(java.sql.Date drawing) {
    this.drawing = drawing;
  }


  public String getCalm() {
    return calm;
  }

  public void setCalm(String calm) {
    this.calm = calm;
  }


  public String getXiuwei() {
    return xiuwei;
  }

  public void setXiuwei(String xiuwei) {
    this.xiuwei = xiuwei;
  }


  public String getExtrapoint() {
    return extrapoint;
  }

  public void setExtrapoint(String extrapoint) {
    this.extrapoint = extrapoint;
  }


  public String getPaysum() {
    return paysum;
  }

  public void setPaysum(String paysum) {
    this.paysum = paysum;
  }


  public String getDaypaysum() {
    return daypaysum;
  }

  public void setDaypaysum(String daypaysum) {
    this.daypaysum = daypaysum;
  }


  public String getDayandpayorno() {
    return dayandpayorno;
  }

  public void setDayandpayorno(String dayandpayorno) {
    this.dayandpayorno = dayandpayorno;
  }


  public String getDayfirstinorno() {
    return dayfirstinorno;
  }

  public void setDayfirstinorno(String dayfirstinorno) {
    this.dayfirstinorno = dayfirstinorno;
  }


  public String getDaygetorno() {
    return daygetorno;
  }

  public void setDaygetorno(String daygetorno) {
    this.daygetorno = daygetorno;
  }


  public String getVipget() {
    return vipget;
  }

  public void setVipget(String vipget) {
    this.vipget = vipget;
  }


  public String getLoworhihtpack() {
    return loworhihtpack;
  }

  public void setLoworhihtpack(String loworhihtpack) {
    this.loworhihtpack = loworhihtpack;
  }


  public String getPals() {
    return pals;
  }

  public void setPals(String pals) {
    this.pals = pals;
  }


  public String getFmsld() {
    return fmsld;
  }

  public void setFmsld(String fmsld) {
    this.fmsld = fmsld;
  }


  public String getAttachpack() {
    return attachpack;
  }

  public void setAttachpack(String attachpack) {
    this.attachpack = attachpack;
  }


  public String getMeridians() {
    return meridians;
  }

  public void setMeridians(String meridians) {
    this.meridians = meridians;
  }


  public String getHjmax() {
    return hjmax;
  }

  public void setHjmax(String hjmax) {
    this.hjmax = hjmax;
  }


  public String getQianDao() {
    return qianDao;
  }

  public void setQianDao(String qianDao) {
    this.qianDao = qianDao;
  }


  public String getOnlineTime() {
    return onlineTime;
  }

  public void setOnlineTime(String onlineTime) {
    this.onlineTime = onlineTime;
  }


  public String getXingpans() {
    return xingpans;
  }

  public void setXingpans(String xingpans) {
    this.xingpans = xingpans;
  }


  public String getTtvictory() {
    return ttvictory;
  }

  public void setTtvictory(String ttvictory) {
    this.ttvictory = ttvictory;
  }


  public String getTtfail() {
    return ttfail;
  }

  public void setTtfail(String ttfail) {
    this.ttfail = ttfail;
  }


  public String getTtrecord() {
    return ttrecord;
  }

  public void setTtrecord(String ttrecord) {
    this.ttrecord = ttrecord;
  }


  public String getTtjiangli() {
    return ttjiangli;
  }

  public void setTtjiangli(String ttjiangli) {
    this.ttjiangli = ttjiangli;
  }


  public String getGmshoptype() {
    return gmshoptype;
  }

  public void setGmshoptype(String gmshoptype) {
    this.gmshoptype = gmshoptype;
  }


  public String getGradeincrease() {
    return gradeincrease;
  }

  public void setGradeincrease(String gradeincrease) {
    this.gradeincrease = gradeincrease;
  }


  public String getCurrentattribute() {
    return currentattribute;
  }

  public void setCurrentattribute(String currentattribute) {
    this.currentattribute = currentattribute;
  }


  public String getGametimeremaining() {
    return gametimeremaining;
  }

  public void setGametimeremaining(String gametimeremaining) {
    this.gametimeremaining = gametimeremaining;
  }

}
